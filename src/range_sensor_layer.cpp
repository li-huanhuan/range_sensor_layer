...
void RangeSensorLayer::reconfigureCB(range_sensor_layer::RangeSensorLayerConfig &config, uint32_t level)
{
  ...
  set_max_range_ = config.max_range;
  set_min_range_ = config.min_range;
  set_angle_  = config.angle;
  marker_duration_ = config.marker_duration;
...
void RangeSensorLayer::bufferIncomingRangeMsg(const sensor_msgs::RangeConstPtr& range_message)
{
  ...
  sensor_msgs::Range range = *range_message;
  if(range.range <= this->set_max_range_ && range.range > this->set_min_range_)
  {
    range.max_range = static_cast<float>(this->set_max_range_);
    range.min_range = static_cast<float>(this->set_min_range_);
    range.field_of_view = std::min(range.field_of_view,static_cast<float>(this->set_angle_));
...
{
  if (fabs(range_message.min_range - range_message.max_range) < 0.0001)
...
void RangeSensorLayer::processFixedRangeMsg(sensor_msgs::Range& range_message)
{
  ...
  {
    ROS_ERROR_THROTTLE(1.0,"Fixed distance ranger (min_range == max_range) in frame %s sent invalid value. "
...
void RangeSensorLayer::processVariableRangeMsg(sensor_msgs::Range& range_message)
{
  ...
  if (fabs(range_message.range - range_message.max_range) < 0.0001 && clear_on_max_reading_)
    clear_sensor_cone = true;
...
{
  max_angle_ = range_message.field_of_view / 2.0; //检测角度的一半
  ...
  {
    setCost(aa, ab, 233); //将range处设为障碍物
...
**********************************/
  ...
  for (unsigned int x = bx0; x <= (unsigned int)bx1; x++)
  {
    for (unsigned int y = by0; y <= (unsigned int)by1; y++)
    {
      ...
      // 除非将inflate_cone_设置为100％，否则我们只会更新（部分膨胀的）传感器圆锥内的单元格，
      ...
      // details
      // 在成本图上投影为三角形。
      // 0％仅对应于三角形，但是如果您的传感器视野非常狭窄，则由于细胞离散化，覆盖区域可能会变为零。
      // 有关更多详细信息，请参见Wiki描述。
      if (inflate_cone_ < 1.0)
      {
        ...
        // 区域阈值内的重心坐标； 从数学上来说这根本不是听起来很合理，但是可以！
      ...
      if (update_xy_cell)
      {
        ...
        update_cell(ox, oy, theta, static_cast<double>(range_message.range), wx, wy, clear_sensor_cone);
        std::pair<double,double> pair_tem(wx, wy);
  ...
  if(!marker_points.points.empty())
  {
    ...
    this->last_marker_points_buffer_.push_back(marker_points);
  }
...
void RangeSensorLayer::clearHistoryObs()
{
   boost::mutex::scoped_lock lock(clear_mutex_);
   ros::Time now = ros::Time::now();
   size_t buffer_size = this->last_marker_points_buffer_.size();
   for(size_t i=0;i<buffer_size;i++)
   {
     if( (now.toSec() - this->last_marker_points_buffer_[i].marker_time.toSec() ) > marker_duration_)
     {
       size_t points_size = this->last_marker_points_buffer_[i].points.size();
       for(size_t j=0;j<points_size;j++)
       {
         unsigned int mx,my;
         worldToMap(last_marker_points_buffer_[i].points[j].first, last_marker_points_buffer_[i].points[j].second, mx ,my);
         setCost(mx,my, costmap_2d::FREE_SPACE);
       }
       this->last_marker_points_buffer_.erase(this->last_marker_points_buffer_.begin()+static_cast<int>(i));
     }
     else
     {
       return;
