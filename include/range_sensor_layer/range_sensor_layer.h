...
public:
  ...
  /*****************************
   * 这在initialize（）的末尾被调用。 重写以实现特定于子类的初始化。
   * 调用时，tf_，name_和layered_costmap_都将被设置。
   ****************************/
 ...
 ******************************/
  ...
  /******************************
   * 实际上仅在UpdateBounds（）期间计算的范围内更新基础成本图。
  *******************************/
...
private:
  ...
  std::vector< MarkerPoints > last_marker_points_buffer_;
  double set_max_range_;
  double set_min_range_;
  double set_angle_;
